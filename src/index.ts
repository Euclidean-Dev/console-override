...
export const ConsoleOverride = {
  configure: (options: Configuration = { enabled: true }) => {
    if (
      !options.enabled ||
      (options.debugKey && readLocalStorageKey(options.debugKey))
    ) {
      return;
    }
    clearOnLoad(options);
...
const overrideConsoleMethods = (methodsToExclude: Methods[] = []) => {
  ...
  for (const [property, type] of Object.entries<Function>(
    console as ConsoleOverride
  )) {
    if (
      type &&
      typeof type === "function" &&
      !excludedMethods.has(property as Methods)
    ) {
      (console as ConsoleOverride)[property] = () => {
        null;
      };
    }
  }
